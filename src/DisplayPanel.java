...
class DisplayPanel extends JPanel {
    ...
    public DisplayPanel(ArbolBinario t) {
...
//        g.setFont(MyFont);
        this.drawTree(g, t.raiz); // draw the tree
        revalidate(); //update the component panel
    }

    public void drawTree(Graphics g, NodoArbol root) {//actually draws the tree
        int dx, dy, dx2, dy2;
        ...
        if (root != null) { // inorder traversal to draw each node
            drawTree(g, root.hijoIzquierdo); // do left side of inorder traversal
            dx = root.xpos * XSCALE; // get x,y coords., and scale them
            dy = root.ypos * YSCALE +ys;
            String s = ((String) root.nombre) + " # " +  root.dato ; //get the word at this node
            g.drawString(s, dx, dy); // draws the word
// this draws the lines from a node to its children, if any
            if(root.hijoIzquierdo!=null){ //draws the line to left child if it exists
                dx2 = root.hijoIzquierdo.xpos * XSCALE;
                dy2 = root.hijoIzquierdo.ypos * YSCALE +ys;
                g.drawLine(dx,dy,dx2,dy2);
            }
            if(root.hijoDerecho!=null){ //draws the line to right child if it exists
                dx2 = root.hijoDerecho.xpos * XSCALE;//get right child x,y scaled position
                dy2 = root.hijoDerecho.ypos * YSCALE + ys;
                g.drawLine(dx,dy,dx2,dy2);
            }
            drawTree(g, root.hijoDerecho); //now do right side of inorder traversal
        }
    }
}
